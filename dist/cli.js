...
function getSourceLabel(url, useLocalFile) {
    if (useLocalFile) {
...
function buildAppDescription(appName, url, useLocalFile) {
...
class BaseBuilder {
    ...
    async buildAndCopy(url, target) {
        ...
        if (this.options.saveBuildCommand) {
            await this.saveBuildCommandArtifact(name, fileType, target);
        }
    ...
    buildReproducibleCommand() {
        const args = process.argv.slice(2).map((arg) => this.shellQuote(arg));
        return `pake ${args.join(' ')}`;
    }
    resolveBuildCommandArtifactPath(name) {
        const optionValue = this.options.saveBuildCommand;
        if (typeof optionValue === 'string') {
            return path.resolve(optionValue);
        }
        return path.resolve(`${name}.build.txt`);
    }
    async saveBuildCommandArtifact(name, fileType, target) {
        const artifactPath = this.resolveBuildCommandArtifactPath(name);
        const command = this.buildReproducibleCommand();
        const lines = [
            '# Generated by Pake CLI',
        ...
        await fsExtra.writeFile(artifactPath, lines.join('\n'));
        logger.success('✔ Build command artifact located in', artifactPath);
    }
...
`;
    return program$1
        ...
        .addOption(new Option('--app-version <string>', 'App version (overrides version scheme when set)')
        .default(DEFAULT_PAKE_OPTIONS.appVersion)
        .hideHelp())
        .addOption(new Option('--version-scheme <string>', 'Version scheme: date (YYYY.MM.patch) or manual')
        .choices(['date', 'manual'])
        .default(DEFAULT_PAKE_OPTIONS.versionScheme)
        .hideHelp())
        .addOption(new Option('--version-patch <number>', 'Patch value for date versioning')
        .default(DEFAULT_PAKE_OPTIONS.versionPatch)
        .argParser((value) => {
        ...
        .hideHelp())
        .addOption(new Option('--save-build-command [path]', 'Save the exact CLI build command for reproducible rebuilds')
        .default(DEFAULT_PAKE_OPTIONS.saveBuildCommand)
        .argParser((value) => {
        if (value === undefined)
            return true;
